...
The v1 desktop UI stack is **gioui** per [`decisions.md`](decisions.md) ADR-001. As of 2026-05-28, `internal/ui` contains a pure state/view-model adapter that converts `app.Session` snapshots into UI-ready command, buffer, view, and project models, a presenter that tracks current state and emits coalesced update signals after editor events, a typed controller that maps future UI intents onto app-owned mutation commands, a shell boundary future windows can drive, and a minimal read-only Gio renderer/window loop behind the `gio` build tag. The renderer uses a test-covered sectioned model for header, project, buffers, commands, command-palette state, and editor-placeholder state. A bounded command-palette interaction shell exists for open/close and keyboard selection navigation, but command execution from the palette is intentionally pending. No usable editor UI is implemented yet: no editor widget, file tree interaction, or production input handling exists. The default binary remains an informational pre-alpha CLI; `kleiber experimental-ui --smoke [path]` is the no-window model verification path available without `-tags=gio`, and `kleiber experimental-ui [path]` is the explicit experimental window path for builds compiled with `-tags=gio`. Manual visual verification for that native window lives in [`docs/contributing/gio-smoke.md`](../contributing/gio-smoke.md). The core remains UI-agnostic and exposes a clear command/event API.
